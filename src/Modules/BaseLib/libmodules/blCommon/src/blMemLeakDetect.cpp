...
// Only compile it on Windows and Debug mode
...
static blMemLeakDetect*	g_pMemTrace			= NULL;
...
void blMemLeakDetect::addMemoryTrace(void* addr,  DWORD asize,  TCHAR *fname, DWORD lnum)
{
	...
	{
		// already allocated
		AfxTrace("ERROR!blMemLeakDetect::addMemoryTrace() Address(0x%08X) already allocated\n", addr);
		return;
	}
	//
	ainfo.address		= addr;
...
void blMemLeakDetect::redoMemoryTrace(void* addr,  void* oldaddr, DWORD asize,  char *fname, DWORD lnum)
{
	AllocBlockInfo ainfo;
	...
	{
		AfxTrace(_T("ERROR!blMemLeakDetect::redoMemoryTrace() didnt find Address(0x%08X) to free\n"), oldaddr);
	}
	//
	ainfo.address		= addr;
	ainfo.lineNumber	= lnum;
...
void blMemLeakDetect::removeMemoryTrace(void* addr, void* realdataptr)
{
	AllocBlockInfo ainfo;
	//
	...
	else
	{
	   //freeing unallocated memory
		AfxTrace(_T("ERROR!blMemLeakDetect::removeMemoryTrace() didnt find Address(0x%08X) to free\n"), addr);
	}
};
void blMemLeakDetect::cleanupMemoryTrace()
{
	m_AllocatedMemoryList.RemoveAll();
};

void blMemLeakDetect::dumpMemoryTrace()
{
	POSITION			pos;
	LPVOID				addr;
	AllocBlockInfo		ainfo;
...
blMemLeakDetect::blMemLeakDetect()
{
	Init();
}
...
// PRIVATE STUFF
void blMemLeakDetect::symbolPaths( TCHAR* lpszSymbolPath)
{
...
BOOL blMemLeakDetect::cleanupSymInfo()
{
	return SymCleanup( GetCurrentProcess() );
}

// Initializes the symbol files
BOOL blMemLeakDetect::initSymInfo( TCHAR* lpszUserSymbolPath )
{
...
void blMemLeakDetect::symStackTrace(STACKFRAMEENTRY* pStacktrace )
{
...
//
void blMemLeakDetect::symStackTrace2(STACKFRAMEENTRY* pStacktrace )
{
	ADDR			FramePtr				= NULL;
...
BOOL blMemLeakDetect::symFunctionInfoFromAddresses( ULONG fnAddress, ULONG stackAddress, LPTSTR lpszSymbol )
{
	DWORD             dwDisp	= 0;
...
BOOL blMemLeakDetect::symSourceInfoFromAddress(UINT address, TCHAR* lpszSourceInfo)
{
	BOOL           ret = FALSE;
...
BOOL blMemLeakDetect::symModuleNameFromAddress( UINT address, TCHAR* lpszModule )
{
	BOOL              ret = FALSE;
...
long blMemLeakDetect::GetNumMemoryLeaks( )
{
	return m_AllocatedMemoryList.GetSize( );
}
