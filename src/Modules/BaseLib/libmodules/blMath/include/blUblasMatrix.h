...
// This class contains an adaptation of the boost ublas matrix type.
// The adaptation was made to allow having a "smart references" class as the reference_type of the matrix,
// something that is not allowed by the original class.
...
#ifndef blUblasMatrix_h
#define blUblasMatrix_h
...
/**
The following helper functions are used to pass the number of matrix columns to the disk-storage policy.
...
namespace Private
{
	//! Call SetNumberOfColumns function of \a storage.
	template< class T >
	void SetNumberOfColumns(blDiskStoragePolicy<T>& storage, size_t n)
	{
		storage.SetNumberOfColumns(n);
	}

	//! For any other storage policy, do nothing.
	template< class S >
	void SetNumberOfColumns(S& storage, size_t n)
	{
	}
}
    ...
    public:
        ...
        matrix (size_type size1, size_type size2):
            ...
            size1_ (size1), size2_ (size2), data_ (layout_type::storage_size (size1, size2)) {
				Private::SetNumberOfColumns(data_, size2_);
        }
        BOOST_UBLAS_INLINE
