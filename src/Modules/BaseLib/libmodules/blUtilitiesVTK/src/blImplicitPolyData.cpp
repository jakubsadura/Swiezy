/*=========================================================================
  ...
  Program:   Boolean
  Module:    $RCSfile: blImplicitPolyData.cpp,v $
  Language:  C++
...
=========================================================================*/
#include "blImplicitPolyData.h"

// VTK
#include <vtkPolygon.h>
#include <vtkObjectFactory.h>

vtkCxxRevisionMacro(blImplicitPolyData, "$Revision: 1.48 $");
vtkStandardNewMacro(blImplicitPolyData);

// Constructor
blImplicitPolyData::blImplicitPolyData()
{
...
void blImplicitPolyData::SetInput(vtkPolyData *input)
{
  if( this->input != input ) 
    {
...
void blImplicitPolyData::SetEvaluateBounds( double eBounds[6] )
{
  int i;
  for( i=0; i<6; i++ ) 
    {
...
unsigned long blImplicitPolyData::GetMTime()
{
...
double blImplicitPolyData::EvaluateFunction(double x[3])
{
...
void blImplicitPolyData::EvaluateGradient( double x[3], double n[3] )
{
  int i;
  // See if data set with polygons has been specified
  if( this->input == NULL || input->GetNumberOfCells() == 0 )
...
void blImplicitPolyData::PrintSelf(ostream& os, vtkIndent indent)
{
