...
blMitkSurfaceSelectInteractor::blMitkSurfaceSelectInteractor(
	const char * type, 
...
: Interactor(type, dataTreeNode)
{
	...
	blMITKUtils::ComputePointSize(meshPointSet,pointSize,relativeSelectedPointSize);
	...
	m_ImplicitSphere =  vtkSmartPointer<vtkSphere>::New();
	m_SphereSource =  vtkSmartPointer<vtkSphereSource>::New();
	m_SphereSource->SetThetaResolution(100);
	m_SphereSource->SetPhiResolution(100);
	m_SphereSource->SetRadius(0);
	m_ImplicitSphere->SetRadius(0);
	m_SphereNode  = sphereTreeNode;
	m_SphereNode->SetVisibility(false);
	mitk::Surface::Pointer	surface = mitk::Surface::New( );
	surface->SetVtkPolyData( m_SphereSource->GetOutput() );
	m_SphereNode->SetData( surface );
...
int blMitkSurfaceSelectInteractor::GetDirection( )
{
	return m_Direction;
}

void blMitkSurfaceSelectInteractor::SetBoundingBox(double bounds[ 6 ]) 
{ 
...
bool blMitkSurfaceSelectInteractor::ExecuteAction( Action* action, mitk::StateEvent const* stateEvent )
{
	...
	case AcADDPOINT:
		{
			...
			if (iPosition>=0)
			{
				...
				m_SphereSource->SetCenter(polyDataPoint[ 0 ],polyDataPoint[ 1 ],polyDataPoint[ 2 ]);
				m_ImplicitSphere->SetCenter(polyDataPoint[ 0 ],polyDataPoint[ 1 ],polyDataPoint[ 2 ]);
				...
				if (m_UndoEnabled)
				{
					...
					PointOperation *undoOp = new mitk::PointOperation(
						OpREMOVE, timeInMS, polyDataPoint, 0);
	...
	case AcUPDATEMESH:
		{
			...
			if (  m_Radius > 0 ) 
			{
				m_SphereSource->SetCenter(qPoint[0],qPoint[1],qPoint[2]);
				m_SphereSource->SetRadius(m_Radius);
				m_ImplicitSphere->SetCenter(qPoint[0],qPoint[1],qPoint[2]);
				m_ImplicitSphere->SetRadius(m_Radius);
			}
			m_SphereNode->SetVisibility(true);
			m_SphereNode->SetOpacity(0.5);
	...
	case AcNEWPOINT:
		{
			...
			double pPoint[ 3 ] = { worldPoint[ 0 ], worldPoint[ 1 ], worldPoint[ 2 ] };
			PointOperation* doOp = new mitk::PointOperation(
				OpINSERT, timeInMS, worldPoint, 1, true, PTUNDEFINED);
...
blMitkSurfaceSelectInteractor::Pointer blMitkSurfaceSelectInteractor::New( 
	const char * type, 
	mitk::DataTreeNode* dataTreeNode, 
	vtkPolyData *meshPointSet,
	mitk::DataTreeNode* sphereTreeNode,
...
{
	Pointer smartPtr = new Self( type, dataTreeNode, meshPointSet,
		sphereTreeNode,points,
...
bool blMitkSurfaceSelectInteractor::GetPickedPoint( 
...
void blMitkSurfaceSelectInteractor::SetRadius(double radius)
{
	m_Radius = radius;
}

double blMitkSurfaceSelectInteractor::GetRadius()
{
	return m_Radius;
}

vtkSphere* blMitkSurfaceSelectInteractor::GetImplicitSphere()
{
	return m_ImplicitSphere;
}

void blMitkSurfaceSelectInteractor::ResetSphere()
{
	m_ImplicitSphere =  vtkSmartPointer<vtkSphere>::New();
	m_SphereSource =  vtkSmartPointer<vtkSphereSource>::New();
	m_SphereSource->SetThetaResolution(100);
