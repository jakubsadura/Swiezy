...
bool wxThumbnailItem::DrawBackground(wxDC& dc, wxThumbnailCtrl* ctrl, const wxRect& rect, const wxRect& imageRect, int style, int WXUNUSED(index))
{
	...
	// Select the text to show in each label
	wxString filename;
	wxString file, name, ext;
	// If extension label is shown -> Hide extension here
	wxString baseText = filename;
	if ( (ctrl->GetWindowStyle() & wxTH_TEXT_LABEL) )
	{
		filename = wxFileNameFromPath(m_filename);
		wxSplitPath(filename, & file, & name, & ext);
		baseText = filename;
	}
	else if ( (ctrl->GetWindowStyle() & wxTH_SHOW_TITLE) )
	{
		filename = m_title;
		wxSplitPath(filename, & file, & name, & ext);
		baseText = name;
    ...
    if ( !baseText.IsEmpty() && ( 
		(ctrl->GetWindowStyle() & wxTH_TEXT_LABEL) ||
		(ctrl->GetWindowStyle() & wxTH_SHOW_TITLE) ) )
    {
