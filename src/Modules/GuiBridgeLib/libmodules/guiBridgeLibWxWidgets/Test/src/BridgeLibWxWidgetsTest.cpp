...
// Modified by:
// Created:     31.01.01
// RCS-ID:      $Id: BridgeLibWxWidgetsTest.cpp,v 1.2 2008-01-28 09:25:59 mnieber Exp $
...
: wxFrame((wxFrame *)NULL, wxID_ANY, title, pos, size)
{
	...
	wxButton *button = new wxButton(this, Event_ButtonPush, "Test");

	wxObjectEventFunction f = (wxObjectEventFunction) &MyFrame::OnDynamic;
	this->Connect(button->GetId(), wxID_ANY, wxEVT_COMMAND_BUTTON_CLICKED, f);

	gbl::Bridge bridge;
	gbl::AddBridgeEvent(bridge, "DoIt", button);

	wxEvtHandler* evtHandler = button->GetEventHandler();
	for( int i = 0; i < 10; ++i )
	{
		evtHandler = button->GetNextHandler();
...
void MyFrame::OnConnect(wxCommandEvent& event)
{
	if ( event.IsChecked() )
	{
		...
		Connect(Event_Dynamic, wxID_ANY, wxEVT_COMMAND_MENU_SELECTED,
			(wxObjectEventFunction) &MyFrame::OnDynamic);
